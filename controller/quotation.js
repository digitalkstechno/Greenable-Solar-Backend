...
const drawSmileyBullet = (doc, x, y) => {
...
const renderMixedLine = (doc, segments, centerX, y, fontSize, baseFontStyle) => {
...
const wrapSegmentsToLines = (doc, segments, maxWidth, fontSize, baseFontStyle) => {
    const words = [];
    segments.forEach(seg => {
...
// ── Core PDF generation function (returns Buffer) ──────────────────────────
const generatePdfBuffer = (q, lead) => {
    ...
    doc.line(rightX, 117, rightX + dateTextWidth, 117);

    currY = 129;
    const prefixWidth = doc.getTextWidth("ADD. – ");
    const alignX = 40 + prefixWidth;
    ...
    const optionsList = q.options?.length ? q.options : ['AMOUNT (INR)'];
    ...
    const tableBody = (q.rows || []).map((row, idx) => {
    ...
    const fixedWidth = 45 + descWidth;
    const availableWidth = pageWidth - 80 - fixedWidth;
    ...
    const bomBody = (q.bomItems || []).map((item) => [
        stripHtml(item.srNo || ''),
...
exports.generateQuotation = async (req, res) => {
    try {
        const { q, lead } = req.body; // frontend e j structure ma q and lead pathavvana

        if (!q || !lead) {
            return res.status(400).json({ status: 'Fail', message: 'q and lead data required' });
        }

        const pdfBuffer = generatePdfBuffer(q, lead);

        const fileName = `Quotation_${lead?.fullName || 'lead'}_${q.id || Date.now()}.pdf`;
